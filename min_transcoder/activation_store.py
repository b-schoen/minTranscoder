# from https://github.com/jacobdunefsky/transcoder_circuits/blob/master/sae_training/activations_store.py
...
class ActivationsStore:
    ...
    while training transcoders.

    This is only needed when we're later looking for top activating examples (or
    when initializing transcoder weights with geometric mean).
    ...
    def __init__(
        ...
        if create_dataloader:
            ...
            if self.cfg.is_transcoder:
                ...
                self.storage_buffer, self.storage_buffer_out = self.get_buffer(
                    self.cfg.n_batches_in_buffer // 2
                )
            else:
    ...
    def get_activations(self, batch_tokens, get_loss=False):
        ...
        assert not (
            self.cfg.is_transcoder and (self.cfg.hook_point_head_index is not None)
        )
    ...
    def get_data_loader(
        ...
        if self.cfg.is_transcoder:
            ...
            new_buffer, new_buffer_out = self.get_buffer(
                self.cfg.n_batches_in_buffer // 2
            )
            ...
            self.storage_buffer_out = mixing_buffer_out[
                : mixing_buffer_out.shape[0] // 2
            ]
            ...
            dataloader = iter(
                DataLoader(catted_buffers, batch_size=batch_size, shuffle=True)
            )
        else:
